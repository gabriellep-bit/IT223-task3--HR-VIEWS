...
(501, 'Kyle', 'Gotera', 'goteraprincesskyle@gmail.com', 'Kyle123', '09708877793', '2026-02-06', 'IT_PROG', 143000, NULL, 102, 60);
...
--
-- Stand-in structure for view `emp_view_jobs`
-- (See below for the actual view)
--
CREATE TABLE `emp_view_jobs` (
`employee_id` int(11) unsigned
,`name` varchar(46)
...
,`Name` varchar(46)
,`Job Title` varchar(35)
,`Employment Date` varchar(23)
,`Manager Name` varchar(46)
...
--
-- Stand-in structure for view `view_address`
-- (See below for the actual view)
--
...
--
-- Structure for view `emp_view_jobs`
--
DROP TABLE IF EXISTS `emp_view_jobs`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `emp_view_jobs`  AS SELECT `e`.`employee_id` AS `employee_id`, concat(`e`.`first_name`,' ',`e`.`last_name`) AS `name`, `j`.`job_title` AS `job_title` FROM (`employees` `e` join `jobs` `j` on(`j`.`job_id` = `e`.`job_id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `hr_employee_view`  AS SELECT `e`.`employee_id` AS `Employee ID`, concat(`e`.`first_name`,' ',`e`.`last_name`) AS `Name`, coalesce(`j`.`job_title`,'N/A') AS `Job Title`, coalesce(concat(min(`jh`.`start_date`),' - ',coalesce(max(`jh`.`end_date`),'Present')),'N/A') AS `Employment Date`, coalesce(concat(`m`.`first_name`,' ',`m`.`last_name`),'No Manager') AS `Manager Name`, coalesce(`d`.`department_name`,'N/A') AS `Department Name`, coalesce(concat(`l`.`street_address`,', ',`l`.`postal_code`,', ',`l`.`city`,', ',`l`.`state_province`,', ',`c`.`country_name`,', ',`r`.`region_name`),'No Location') AS `Location` FROM (((((((`employees` `e` left join `employees` `m` on(`e`.`manager_id` = `m`.`employee_id`)) left join `jobs` `j` on(`e`.`job_id` = `j`.`job_id`)) left join `job_history` `jh` on(`e`.`employee_id` = `jh`.`employee_id`)) left join `departments` `d` on(`e`.`department_id` = `d`.`department_id`)) left join `locations` `l` on(`d`.`location_id` = `l`.`location_id`)) left join `countries` `c` on(`l`.`country_id` = `c`.`country_id`)) left join `regions` `r` on(`c`.`region_id` = `r`.`region_id`)) GROUP BY `e`.`employee_id` ;

-- --------------------------------------------------------

--
-- Structure for view `view_address`
--
DROP TABLE IF EXISTS `view_address`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_address`  AS SELECT concat(`l`.`street_address`,' ',`l`.`postal_code`,' ',`l`.`city`,' ',`l`.`state_province`,' ') AS `address`, `c`.`country_name` AS `country_name`, `r`.`region_name` AS `region_name` FROM ((`locations` `l` join `countries` `c` on(`l`.`country_id` = `c`.`country_id`)) join `regions` `r` on(`r`.`region_id` = `c`.`region_id`)) ;
...
ALTER TABLE `employees`
  ...
  ADD UNIQUE KEY `idx_unique_name` (`first_name`,`last_name`);
